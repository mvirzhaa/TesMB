...
import { 
  ...
  ThumbsUp, ThumbsDown, Lock, Mail, Phone, AlertTriangle, Layout, Activity, Sparkles, BrainCircuit, ArrowLeft, GraduationCap
} from 'lucide-react';
...
function App() {
  ...
  // State untuk Konten Dinamis (Jurusan & Karir)
  const [riasecContent, setRiasecContent] = useState({});

  useEffect(() => {
    // Ambil data rekomendasi dari database saat app dibuka
  ...
  const handleStart = async (e) => {
    ...
    if (lastResult) {
        ...
        if (!isFinished) {
            if (window.confirm(`Hai ${lastResult.user_name}, kamu belum menyelesaikan tes sebelumnya (Halaman ${lastResult.last_page + 1}). \n\nTekan OK untuk LANJUTKAN.\nTekan CANCEL untuk MULAI BARU.`)) {
  ...
  const renderQuestionCard = (q, globalIndex, myAnswer) => {
     ...
     switch(q.type) {
        ...
        case 'star': 
          return (
            <div className="flex flex-col items-center justify-center py-6 bg-slate-50 rounded-xl border border-slate-100 mt-4">
               <div className="flex gap-3">
                 {[1,2,3,4,5].map(v => (
                   <button key={v} onClick={() => handleAnswer(globalIndex, v, q.category_main)} 
                     className={`p-1 transition-all duration-200 hover:scale-110 ${myAnswer?.weight >= v ? 'text-amber-400 drop-shadow-sm' : 'text-slate-200 hover:text-amber-200'}`}>
                     <Star size={38} fill={myAnswer?.weight >= v ? "currentColor" : "none"} strokeWidth={myAnswer?.weight >= v ? 0 : 1.5} />
                   </button>
                 ))}
               </div>
               <div className="mt-3 text-xs font-bold text-slate-400 uppercase tracking-widest">{myAnswer?.weight ? `Rating: ${myAnswer.weight}/5` : 'Berikan Rating'}</div>
            </div>
          );
        case 'boolean': 
          return (
            <div className="flex gap-4 mt-4">
               <button onClick={() => handleAnswer(globalIndex, 1, q.category_main)} 
                  className={`flex-1 py-4 rounded-xl border-2 flex items-center justify-center gap-2 font-bold transition-all ${myAnswer?.weight === 1 ? 'border-rose-500 bg-rose-50 text-rose-600 shadow-md ring-2 ring-rose-200' : 'border-slate-100 text-slate-400 hover:border-rose-200 hover:text-rose-400'}`}>
                  <ThumbsDown size={20}/> TIDAK
               </button>
               <button onClick={() => handleAnswer(globalIndex, 5, q.category_main)} 
                  className={`flex-1 py-4 rounded-xl border-2 flex items-center justify-center gap-2 font-bold transition-all ${myAnswer?.weight === 5 ? 'border-teal-500 bg-teal-50 text-teal-600 shadow-md ring-2 ring-teal-200' : 'border-slate-100 text-slate-400 hover:border-teal-200 hover:text-teal-400'}`}>
                  <ThumbsUp size={20}/> YA
               </button>
            </div>
          );
        case 'choice': 
          return (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4">
               {[
                 { val: 2, cat: q.category_main, text: q.text_main, label: 'A', color: 'indigo' },
                 { val: 2, cat: q.category_pair, text: q.text_pair, label: 'B', color: 'pink' }
               ].map((opt, i) => {
                 const isActive = myAnswer?.weight === opt.val && myAnswer?.category === opt.cat;
                 return (
                   <button key={i} onClick={() => handleAnswer(globalIndex, opt.val, opt.cat)} 
                      className={`p-5 rounded-xl border-2 text-left transition-all relative overflow-hidden ${isActive ? `border-${opt.color}-500 bg-${opt.color}-50 shadow-md ring-2 ring-${opt.color}-200` : 'border-slate-100 hover:border-slate-300 bg-white'}`}>
                      <span className="text-xs font-black text-slate-400 mb-1 block tracking-widest uppercase">Pilihan {opt.label}</span>
                      <span className={`text-sm font-bold ${isActive ? `text-${opt.color}-900` : 'text-slate-600'}`}>{opt.text}</span>
                      {isActive && <div className={`absolute top-4 right-4 text-${opt.color}-600`}><CheckCircle size={18} /></div>}
                   </button>
                 )
               })}
            </div>
  ...
  // --- HALAMAN 1: INTRO (Clean & Modern) ---
  if (currentStep === 'intro') {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 font-sans">
        <div className="max-w-5xl w-full bg-white rounded-[2rem] shadow-xl overflow-hidden flex flex-col md:flex-row border border-slate-100">
           <div className="flex-1 p-8 md:p-12 flex flex-col justify-center">
              <div className="mb-8">
                 <div className="inline-flex items-center gap-2 px-3 py-1 bg-indigo-50 text-indigo-600 rounded-full text-xs font-bold uppercase tracking-wider mb-4"><Sparkles size={14}/> Psikometri Online</div>
                 <h1 className="text-4xl font-extrabold text-slate-800 mb-2">Tes Minat & Bakat</h1>
                 <p className="text-slate-500">Temukan potensi karir dan kepribadianmu dalam 138 pertanyaan.</p>
              </div>
           ...
           <div className="flex-1 bg-indigo-900 text-white p-8 md:p-12 flex flex-col justify-center relative overflow-hidden">
              <div className="relative z-10">
                 <h3 className="text-xl font-bold mb-6 flex items-center gap-2"><AlertTriangle className="text-yellow-400"/> Ketentuan Tes</h3>
                 <ul className="space-y-4 text-sm text-indigo-100">
                    <li className="flex gap-3"><div className="w-6 h-6 rounded-full bg-indigo-700 flex items-center justify-center text-xs font-bold">1</div> <span>Terdiri dari <strong>138 Soal</strong> (Minat & Kepribadian).</span></li>
                    <li className="flex gap-3"><div className="w-6 h-6 rounded-full bg-indigo-700 flex items-center justify-center text-xs font-bold">2</div> <span>Tidak ada jawaban benar/salah. Jujurlah.</span></li>
                    <li className="flex gap-3"><div className="w-6 h-6 rounded-full bg-indigo-700 flex items-center justify-center text-xs font-bold">3</div> <span>Data tersimpan otomatis (Auto-save).</span></li>
                    <li className="flex gap-3"><div className="w-6 h-6 rounded-full bg-indigo-700 flex items-center justify-center text-xs font-bold">4</div> <span>Gunakan email yang sama untuk melanjutkan tes.</span></li>
                 </ul>
              </div>
              ...
              <div className="absolute bottom-0 left-0 w-64 h-64 bg-pink-600 rounded-full mix-blend-overlay filter blur-[50px] opacity-30"></div>
           </div>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen bg-slate-50 font-sans pb-32">
         ...
         <div className="max-w-3xl mx-auto px-4 mt-8 space-y-6">
           {currentQuestions.map((q, idx) => (
              <div key={q.id} className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-slate-200 hover:border-indigo-200 transition-colors">
                 <div className="flex items-start gap-4">
                    <span className="text-slate-300 text-lg font-black mt-0.5">#{startIdx + idx + 1}</span>
                    <h3 className="text-lg font-bold text-slate-800 leading-snug flex-1">{q.text_main}</h3>
                 </div>
                 <div className="pl-0 md:pl-10">{renderQuestionCard(q, startIdx + idx, userAnswers[startIdx + idx])}</div>
              </div>
           ))}
         </div>
         <div className="fixed bottom-0 w-full bg-white/90 backdrop-blur-md border-t border-slate-200 p-4 z-40">
            <div className="max-w-3xl mx-auto flex justify-between items-center">
               <div className="hidden md:block text-xs text-slate-400 italic font-medium">Jawaban tersimpan otomatis.</div>
               <button onClick={handleNextPage} disabled={!isPageComplete || isLoading} className={`ml-auto px-8 py-3 rounded-xl font-bold text-white shadow-lg transition-all transform hover:-translate-y-1 flex items-center gap-2 ${!isPageComplete ? 'bg-slate-300 shadow-none cursor-not-allowed opacity-70' : 'bg-indigo-600 hover:bg-indigo-700'}`}>{isLoading ? <Loader2 className="animate-spin"/> : (currentPage === totalPages - 1 ? 'Kumpulkan Jawaban' : 'Selanjutnya')} {!isLoading && <ChevronRight size={18}/>}</button>
            </div>
         </div>
      </div>
    );
  ...
  // --- HALAMAN 3: RESULT (Dashboard Style + Database Content) ---
     ...
     return (
        <div className="min-h-screen bg-slate-50 py-12 px-4 font-sans">
           <div className="max-w-5xl mx-auto space-y-6">
              ...
              <div className="flex justify-center gap-2 p-1.5 bg-white rounded-xl shadow-sm border border-slate-200 w-fit mx-auto">
              ...
              <div className="text-center pt-8 pb-20">
                 <button onClick={()=>window.location.reload()} className="px-8 py-3 bg-slate-800 text-white rounded-xl font-bold hover:bg-slate-900 transition flex items-center gap-2 mx-auto"><RefreshCcw size={18}/> Selesai & Keluar</button>
              </div>
           </div>
        </div>
     );
